...
load_deployment_env() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    value=${line#*=}
    case "$key" in
      GATEWAY_BIND_ADDRESS|GATEWAY_PORT|GATEWAY_URL|CODEX_VERSION|IMAGE_TAG|WORKSPACE_ROOT|CODEX_HOME_HOST|VOLUME_SUFFIX|GATEWAY_CODEX_APPROVAL_POLICY|GATEWAY_CODEX_SANDBOX|GATEWAY_CODEX_MODEL|GATEWAY_CODEX_TURN_TIMEOUT) ;;
      *) continue ;;
    esac
